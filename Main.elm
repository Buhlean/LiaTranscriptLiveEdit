...
type Name_Tag = None | Unavailable | Tag String
              ...
              case parse_list fullList of 
                ...
                Just options -> 
                  case figure_out_which_from_these options of
                    None        -> (model, Http.get { url = "https://video.google.com/timedtext?v=" ++ model.current_id ++ "&lang=en", expect = Http.expectString GotCues })
                    Unavailable -> ({model | state = Load_Failed }        ,Cmd.none)
                    Tag which   -> (model, Http.get { url = "https://video.google.com/timedtext?v=" ++ model.current_id ++ "&lang=en&name=" ++ which, expect = Http.expectString GotCues })
...
figure_out_which_from_these : List Option -> Name_Tag
figure_out_which_from_these options = 
  let
    only_english = List.filter (\r -> r.lang_code == "en") (options)
  in case only_english of
    [] ->                        Unavailable
    [the_one] -> 
      if the_one.name == "" then None
      else                       Tag the_one.name
    x::xs ->                     Tag x.name
...
extract_option node =
  case node of
    Element _ ( id :: name :: lang_code :: lang_original :: lang_translated :: xs) _ ->
      { id              = (id.value)
      , name            = (name.value)
...
empty_option : Option
empty_option = {id="", name="", lang_code="", lang_original="", lang_translated=""}
